...
public class Maze {
  ...
  //fields added by myself
  protected DisjointSets items;
  ...
  public Maze(int horizontalSize, int verticalSize) {
     ...
     **/
    items = new DisjointSets(horiz*vert);
    int[] wallGroup = new int[horiz*vert*2 - horiz - vert];
    int k = 0;
    for (j = 0; j<vert-1; j++) {//horizantal walls
      for (i = 0; i< horiz;i++) {
        wallGroup[k] = i + j*horiz;
        k++;
      }
    }
    for (j = 0; j<vert; j++) {
      for (i = 0; i < horiz-1; i++) {
        wallGroup[k] = -(i + j*(horiz-1));
        k++;
      }
    }

    int w = horiz*vert*2 - horiz - vert;
    while(w > 0){
      int r = randInt(w);//max is w-1
      int middle = wallGroup[r];
      wallGroup[r] = wallGroup[w-1];
      wallGroup[w-1] = middle;
      w--;
    }
    //wallgroup is in random order now!



    for (i = 0;i< horiz*vert*2 - horiz - vert; i++) {
      int order = wallGroup[i];
      int x,y;
      // System.out.println(wallGroup[i]);
      if (wallGroup[i]>0) {//then it is a horizontal wall
        x = order%horiz;
        y = order/horiz;
        // System.out.println("("+x+","+y+")");
        int above = x+y*horiz;
        int below = x+(y+1)*horiz;
        if (items.find(above)!=items.find(below)) {
          items.union(above, below);
          hWalls[x][y] = false;
        }
        ...
        // System.out.println("("+x+","+y+")");
        int left = x+y*horiz;
        int right = x+1+y*horiz;
        if (items.find(left)!=items.find(right)) {
          items.union(left, right);
          vWalls[x][y] = false;
        }
      }
    }
